...
const PLANETS: Record<string, KeplerElements> = {
  Mercury: { a: 0.387098, e: 0.205630, I: 7.0049,   L: 252.2508, L_rate: 4.092334436,  varpi: 77.4564,   Omega: 48.33167  },
  Venus:   { a: 0.723332, e: 0.006773, I: 3.3947,   L: 181.9797, L_rate: 1.602130224,  varpi: 131.53298, Omega: 76.68069  },
  Earth:   { a: 1.000000, e: 0.016708, I: 0.0,      L: 100.4643, L_rate: 0.9856002585, varpi: 102.94719, Omega: 0.0       },
  Mars:    { a: 1.523662, e: 0.093412, I: 1.8506,   L: 355.4533, L_rate: 0.5240207766, varpi: 336.0408,  Omega: 49.57854  },
  Jupiter: { a: 5.203363, e: 0.048393, I: 1.3053,   L: 34.40438, L_rate: 0.0830853001, varpi: 14.75385,  Omega: 100.55615 },
  Saturn:  { a: 9.537070, e: 0.054150, I: 2.48446,  L: 49.94432, L_rate: 0.0333717830, varpi: 92.43194,  Omega: 113.71504 },
};
...
function getHeliocentricCartesian(el: KeplerElements, d: number): [number, number, number] {
  ...
  return [
    x_orb * (cos_om * cos_Om - sin_om * sin_Om * cos_I) - y_orb * (sin_om * cos_Om + cos_om * sin_Om * cos_I),
    x_orb * (cos_om * sin_Om + sin_om * cos_Om * cos_I) - y_orb * (sin_om * sin_Om - cos_om * cos_Om * cos_I),
    x_orb * (sin_om * sin_I) + y_orb * (cos_om * sin_I),
  ];
}
...
function getMoonEcef(jd: number): [number, number, number] {
  ...
  const eci = eclipticToEquatorial(
    [r * cos_beta * Math.cos(lambda_rad), r * cos_beta * Math.sin(lambda_rad), r * Math.sin(beta_rad)],
    getObliquity(d),
  );
...
export function getCelestialPositions(
  ...
  const sunRho: [number, number, number] = [
    sunDir[0] * AU_KM - obsEcef[0],
    sunDir[1] * AU_KM - obsEcef[1],
    sunDir[2] * AU_KM - obsEcef[2],
  ];
  const sunAzel = ecefRhoToAzel(sunRho, obsLat, obsLon);
  results.push({ name: "Sol", az_deg: parseFloat(sunAzel.az_deg.toFixed(1)), el_deg: parseFloat(sunAzel.el_deg.toFixed(1)), icon: "☀️" });

  // Moon
  const moonEcef = getMoonEcef(jd);
  const moonAzel = ecefRhoToAzel(
    [moonEcef[0] - obsEcef[0], moonEcef[1] - obsEcef[1], moonEcef[2] - obsEcef[2]],
    obsLat, obsLon,
  );
  results.push({ name: "Luna", az_deg: parseFloat(moonAzel.az_deg.toFixed(1)), el_deg: parseFloat(moonAzel.el_deg.toFixed(1)), icon: "🌙" });

  // Planets
  const earthHelio = getHeliocentricCartesian(PLANETS["Earth"]!, d);
  ...
  for (const [key, elements] of Object.entries(PLANETS)) {
    ...
    const geoEclKm: [number, number, number] = [
      (helio[0] - earthHelio[0]) * AU_KM,
      (helio[1] - earthHelio[1]) * AU_KM,
      (helio[2] - earthHelio[2]) * AU_KM,
    ];

    const ecef = rot3(gmst82(jd), eclipticToEquatorial(geoEclKm, eps));
    const azel = ecefRhoToAzel(
      [ecef[0] - obsEcef[0], ecef[1] - obsEcef[1], ecef[2] - obsEcef[2]],
      obsLat, obsLon,
    );

    const info = PLANET_SPANISH_NAMES[key]!;
    results.push({ name: info.name, az_deg: parseFloat(azel.az_deg.toFixed(1)), el_deg: parseFloat(azel.el_deg.toFixed(1)), icon: info.icon });
  }
